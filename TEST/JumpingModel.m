% function [joint1, joint2] = JumpingModel(dt)
...
result.len_jump = [];
for i = 1:length(result.len)
    if result.len(i) ~= robot.l
        result.len_jump = [result.len_jump; result.len(i)];
    end
end

result.theta1 = acos((-robot.l2^2+robot.l1^2+result.len_jump.^2)./(2*result.len_jump*robot.l1));
% result.theta2 = pi-result.theta1*2;
result.theta2 = acos((-result.len_jump.^2+robot.l1^2+robot.l2^2)./(2*robot.l1*robot.l2));
...
joint1 = transpose(joint1);
joint2 = transpose(joint2);

writematrix(round(joint1,4),"joint1.txt");
writematrix(round(joint2,4),"joint2.txt");
